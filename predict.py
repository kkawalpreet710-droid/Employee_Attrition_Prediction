...
def show():
    ...
    with form_col:
        with st.container():
            st.markdown("""<div style='background:#fff;border:1px solid #e0d9ce;
                border-radius:16px;padding:1.5rem;'>""", unsafe_allow_html=True)

            st.markdown("<p style='font-size:10px;color:#FFD84D;background:#1a1a1a;display:inline-block;padding:3px 10px;border-radius:4px;text-transform:uppercase;letter-spacing:2px;font-weight:700;'>Personal Info</p>", unsafe_allow_html=True)
            ...
            st.markdown("<p style='font-size:10px;color:#FFD84D;background:#1a1a1a;display:inline-block;padding:3px 10px;border-radius:4px;text-transform:uppercase;letter-spacing:2px;font-weight:700;margin-top:0.5rem;'>Job Details</p>", unsafe_allow_html=True)
            ...
            st.markdown("<p style='font-size:10px;color:#FFD84D;background:#1a1a1a;display:inline-block;padding:3px 10px;border-radius:4px;text-transform:uppercase;letter-spacing:2px;font-weight:700;margin-top:0.5rem;'>Tenure</p>", unsafe_allow_html=True)
    ...
    with result_col:
        ...
        st.markdown(f"""
        <div style='background:{card_bg};border:1px solid #2a2a2a;
            ...
            <div style='text-align:center;margin-bottom:1.5rem;'>
                <div style='font-size:56px;font-weight:900;color:{ring_color};
                    line-height:1;'>{score}%</div>
                <div style='font-size:11px;color:#666;margin-top:4px;
                    margin-bottom:1rem;'>attrition risk score</div>

                <div style='background:#222;border-radius:8px;height:10px;
                    margin:0 auto 1rem;max-width:200px;'>
                    <div style='width:{score}%;height:10px;border-radius:8px;
                        background:{ring_color};transition:width 0.3s;'></div>
                </div>

                <span style='background:{badge_bg};color:{ring_color};
                    font-size:12px;font-weight:800;padding:6px 20px;
            ...
            <div style='font-size:10px;color:#aaa;text-transform:uppercase;
                letter-spacing:1px;margin:1.2rem 0 0.8rem;border-top:1px solid #2a2a2a;
                padding-top:1rem;'>Key risk drivers</div>
        """, unsafe_allow_html=True)
        ...
        if drivers:
            for name, level, desc in drivers:
                ...
                <div style='display:flex;justify-content:space-between;
                    ...
                    <div>
                        <div style='font-size:11px;color:#ddd;'>{name}</div>
                        <div style='font-size:10px;color:#777;margin-top:2px;'>{desc}</div>
                    </div>
            ...
            st.markdown("<p style='font-size:11px;color:#666;'>No significant risk factors detected.</p>", unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div style='background:#111;border:1px solid #2a2a2a;
                ...
                <div style='font-size:10px;color:#FFD84D;text-transform:uppercase;
                    ...
                    💡 Recommendation</div>
                <div style='font-size:12px;color:#bbb;line-height:1.6;'>{rec}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
